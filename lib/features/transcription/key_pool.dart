...
class KeyPool extends ChangeNotifier {
  ...
  /// Вызывается при пробуждении — раздаёт разблокированные ключи ожидающим.
  ///
  /// Выдаёт не более одного waiter'а на каждый РАЗНЫЙ живой ключ за один проход:
  /// это обслуживает всех, кому ключей хватает, и при этом не раздаёт один и тот
  /// же только что разблокированный ключ сразу всем ожидающим («шторм 429»).
  ///
  /// Если после раздачи ожидающие остались (живых ключей не хватило) —
  /// перепланирует пробуждение на ближайшую разблокировку. Без этого пере-арминга
  /// при одновременной разблокировке нескольких ключей второй+ waiter зависал до
  /// 10-мин таймаута: прежняя реализация обслуживала ровно одного и выходила (WR-07).
  void _onWakeup() {
    // Раздаём каждый живой ключ ровно одному ожидающему. handedOut не даёт
    // выдать один и тот же ключ дважды за проход (round-robin при единственном
    // живом ключе вернул бы его снова — это и есть «шторм 429»).
    final handedOut = <String>{};
    while (_waiters.isNotEmpty) {
      final alive = _nextAlive();
      if (alive == null) break; // живых ключей не осталось
      if (!handedOut.add(alive)) break; // все живые ключи уже розданы в этом проходе
      _waiters.removeAt(0).complete(alive);
    }
    // Остались ожидающие — перепланировать пробуждение на ближайшую разблокировку.
    if (_waiters.isNotEmpty) _scheduleWakeup();
    notifyListeners();
  }
